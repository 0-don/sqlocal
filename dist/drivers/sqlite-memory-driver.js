...
export class SQLiteMemoryDriver {
    ...
    async execBatch(statements, method = 'transaction') {
        if (!this.db)
            throw new Error('Driver not initialized');
        const results = [];
        this.db[method]((tx) => {
        ...
        switch (fn.type) {
            ...
            case 'window':
                this.db.createFunction({
                    name: fn.name,
                    xStep: (_, ...args) => fn.func.step(...args),
                    xValue: (_, ...args) => fn.func.value(...args),
                    xInverse: (_, ...args) => fn.func.inverse(...args),
                    xFinal: (_, ...args) => fn.func.final(...args),
                    arity: -1,
                });
                break;
    ...
    async clear() { }
    async purgeOrphans() {
        // In-memory driver has no on-disk artifacts. Nothing to purge.
        return [];
    }
    async destroy() {
        this.closeDb();
